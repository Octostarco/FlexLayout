...
const SharedWorker_1 = require("../SharedWorker");
const defaultIcons = {
...
class Layout extends React.Component {
    constructor(props) {
        ...
        this.onDragMove = (event) => {
            ...
            if ((this.isMouseOutsideLeft(clientRect, event.clientX) ||
        ...
        this.onDragEnd = (event) => {
            ...
            if (this.draggingOutOfWindowBounds && this.dragNode && this.dropInfo) {
                this.doAction(Actions_1.Actions.deleteTab((_a = this.dragNode) === null || _a === void 0 ? void 0 : _a.getId()));
                this.prepareAndPostSharedWorkerMessage(event, true);
                return;
            }
            ...
            if (this.dropInfo) {
                if (this.customDrop) {
                    ...
                    this.doAction(Actions_1.Actions.moveNode(this.dragNode.getId(), this.dragNode, this.dropInfo.node.getId(), this.dropInfo.location, this.dropInfo.index));
                }
            }
            this.setState({ showHiddenBorder: DockLocation_1.DockLocation.CENTER });
        };
    ...
    instantiateSharedWorkerInstance() {
        ...
        this._worker = new SharedWorker(new URL("../SharedWorker.js", import.meta.url));
        (_a = this._worker) === null || _a === void 0 ? void 0 : _a.port.start();
    }
    /** @internal */
    styleFont(style) {
        if (this.props.font) {
    ...
    initializeDrag(e) {
        ...
        this.dragNode = TabNode_1.TabNode._fromJson(e.data.dragNode, this.props.model, false);
        // Transform the received drag rectangle data into a Rect instance
        const receivedRect = e.data.dragRect;
        const rect = new Rect_1.Rect(receivedRect.x, receivedRect.y, receivedRect.width, receivedRect.height);
        // Set the position and size of the tab
        this.dragNode._setTabRect(rect);
        ...
        this.moveTabWithDragAndDrop(this.dragNode, e.data.dragNode.name, event);
    }
    /**
    ...
    cancelDragOutsideBounds() {
        ...
        (_a = this._worker) === null || _a === void 0 ? void 0 : _a.port.postMessage({ messageType: SharedWorker_1.WorkerMessageType.NegativePingResponse, id: this.id });
     ...
     */
    isMouseEventWithinDesiredBounds(clientX, clientY) {
    ...
    handleWorkerMessage(e) {
        ...
        if (e.data.messageType === SharedWorker_1.WorkerMessageType.PositivePingResponse) {
            this.listenerLayoutId = e.data.id;
            return;
        }
        // Handle negative ping response from the worker
        if (e.data.messageType === SharedWorker_1.WorkerMessageType.NegativePingResponse) {
        ...
        if (this.isMouseEventWithinDesiredBounds(messageData.clientX, messageData.clientY)) {
            ...
            switch (messageData.messageType) {
                case SharedWorker_1.WorkerMessageType.Ping:
                    (_a = this._worker) === null || _a === void 0 ? void 0 : _a.port.postMessage({ messageType: SharedWorker_1.WorkerMessageType.PositivePingResponse, id: this.id });
                    break;
                case SharedWorker_1.WorkerMessageType.InitDrag:
                    DragDrop_1.DragDrop.instance.startX = 0;
                    DragDrop_1.DragDrop.instance.startY = 0;
                    this.initializeDrag(e);
                    break;
                case SharedWorker_1.WorkerMessageType.CoordinatesUpdate:
                    ...
                    if (event) {
                        DragDrop_1.DragDrop.instance._onMouseMove(event);
                    }
                    break;
                case SharedWorker_1.WorkerMessageType.Drop:
                    if (DragDrop_1.DragDrop.instance.isDragging()) {
                        DragDrop_1.DragDrop.instance._onMouseUp(e);
                        DragDrop_1.DragDrop.instance.startX = 0;
                        DragDrop_1.DragDrop.instance.startY = 0;
                        this.externalDragStarted = false;
                        // Reset the listener layout ID and drag initialization state
        ...
        else if (messageData.messageType === SharedWorker_1.WorkerMessageType.InitDrag || messageData.messageType === SharedWorker_1.WorkerMessageType.CoordinatesUpdate) {
     ...
     * @param event
     */
    moveTabWithDragAndDrop(node, dragText, event) {
     ...
     */
    prepareAndPostSharedWorkerMessage(event, isDropEvent = false) {
        if (!this._worker) {
            return;
        }
        ...
        if (!event && isDropEvent) {
            this.postWorkerMessage({
                id: this.id,
                messageType: SharedWorker_1.WorkerMessageType.Drop,
            });
            return;
        }
        ...
        const posEvent = DragDrop_1.DragDrop.instance._getLocationEvent(event);
        // If no listenerLayoutId is set, send a Ping message to initiate drag
        if (!this.listenerLayoutId) {
            this.dragInitialised = false;
            this.postWorkerMessage({
                id: this.id,
                messageType: SharedWorker_1.WorkerMessageType.Ping,
        ...
        // If listenerLayoutId exists but drag is not initialised, send an InitDrag message
            ...
            const data = {
                messageType: SharedWorker_1.WorkerMessageType.InitDrag,
                id: this.id,
                dragNode: node === null || node === void 0 ? void 0 : node.toJson(),
        ...
        // If listenerLayoutId exists and drag is initialised but it's not a drop event, send a CoordinatesUpdate message
            ...
            const data = {
                id: this.id,
                messageType: SharedWorker_1.WorkerMessageType.CoordinatesUpdate,
                clientX: posEvent.clientX + window.screenX,
                clientY: posEvent.clientY,
                originScreenX: window.screenX,
                originScreenY: window.screenY,
    ...
    postWorkerMessage(data) {
        var _a;
        (_a = this._worker) === null || _a === void 0 ? void 0 : _a.port.postMessage(data);
    }
    /**
     ...
     */
    deserializeMouseEvent(clientX, clientY, originScreenX, originScreenY, originInnerWidth, originInnerHeight, serializedEvent) {
        if (serializedEvent) {
        ...
        const calculateMouseEventWidthAndHeight = () => {
            ...
            if (this.isToLeft(originScreenX, originInnerWidth, window.screenX)) {
                ...
                finalX = clientX - DragDrop_1.DragDrop.instance.startX;
        ...
        const event = new MouseEvent(this.eventType, Object.assign(Object.assign({}, this.eventData), { clientX: finalX, clientY: finalY }));
        if (serializedEvent) {
     ...
     */
    isToLeft(originScreenX, originInnerWidth, currentScreenX) {
     ...
     */
    isToRight(originScreenX, currentInnerWidth, currentScreenX) {
     ...
     */
    isAbove(originScreenY, originInnerHeight, currentScreenY) {
     ...
     */
    isBelow(originScreenY, currentScreenY, currentInnerHeight) {
