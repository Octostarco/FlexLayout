    ...
    doAction(action) {
        ...
        switch (action.type) {
            ...
            case Actions_1.Actions.MOVE_NODE: {
                const fromNodeById = this._idMap[action.data.fromNodeId];
                if (fromNodeById instanceof TabNode_1.TabNode || fromNodeById instanceof TabSetNode_1.TabSetNode) {
                    const toNode = this._idMap[action.data.toNode];
                    if (toNode instanceof TabSetNode_1.TabSetNode || toNode instanceof BorderNode_1.BorderNode || toNode instanceof RowNode_1.RowNode) {
                        toNode.drop(fromNodeById, DockLocation_1.DockLocation.getByName(action.data.location), action.data.index, action.data.select);
                    }
                }
                else {
                    const fromNode = action.data.fromNode;
